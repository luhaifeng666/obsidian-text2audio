...
import { generateSettings, generateVoice, getVoiceName, getLocalData, getVoices, generateNotice, generateNoticeText } from "./utils";
...
export default class Text2Audio extends Plugin {
  ...
  async onload() {
    ...
    this.registerEvent(this.app.workspace.on('editor-menu', ((menu, _, markdownView) => {
      (activeWindow.getSelection() || "").toString().replace(/\s/g, '').length > 0 && menu.addItem((item) => {
        item
          .setTitle("Read the selected text")
          .setIcon("audio-file")
          .onClick(() => {
            this.play(markdownView.editor?.getSelection() || "");
          });
      });
    })));
  ...
  async play(text: string) {
    const { key, region } = this.settings;
    const regionCode: string = getLocalData("region") || LANGUAGES[0].region;
